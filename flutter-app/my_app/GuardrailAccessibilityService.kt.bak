...
class GuardrailAccessibilityService : AccessibilityService() {
    ...
    private val backendUrl = "https://cyberbully-check.tail39b525.ts.net/analyze-threat"
    ...
    private fun getChildId(): String {
        ...
        return prefs.getString("flutter.child_id", "demo_child_01") ?: "demo_child_01"
    }

    private fun sendToBackend(text: String, senderApp: String, childId: String) {
        try {
            ...
            json.put("sender_app", senderApp)

            conn.outputStream.use { it.write(json.toString().toByteArray()) }

            val responseCode = conn.responseCode
            Log.d("GuardrailNative", "Backend responded: $responseCode")
            conn.disconnect()
        } catch (e: Exception) {
            Log.e("GuardrailNative", "Failed to reach backend: ${e.message}")
        }
    }
    ...
    private fun processScreenContent(packageName: String) {
        ...
        if (extracted.isNotEmpty() && extracted != lastCapturedText) {
            ...
            // Skip Flutter and send directly to the backend in the background!
            val childId = getChildId()
            Thread {
                sendToBackend(extracted, packageName, childId)
            }.start()
        }
    }
